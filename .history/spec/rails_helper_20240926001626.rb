...
RSpec.configure do |config|
  config.before(:each) do
    allow_any_instance_of(TokenBlacklistService).to receive(:blacklist).and_return(true)
    allow_any_instance_of(TokenBlacklistService).to receive(:blacklisted?).and_return(false)
  end
end
